...
var HC={apikey:K,Authorization:"Bearer "+K,"Content-Type":"application/json"};
var _p=1,_ps=100,_f={},_s="skor.desc.nullslast",_sel=new Set(),_d=[];

function T(m,t){
  var e=document.createElement("div");
  var c=t==="e"?"#ef4444":t==="w"?"#f59e0b":"#22c55e";
  e.style.cssText="position:fixed;top:20px;right:20px;background:#1a1a24;border:2px solid "+c+";border-radius:10px;padding:12px 18px;z-index:99999;font-size:13px;color:#f0ede8;box-shadow:0 8px 24px rgba(0,0,0,.5);max-width:300px";
  e.textContent=m;document.body.appendChild(e);
  setTimeout(function(){e.remove();},3000);
}

function IB(v){
  if(v==="yuksek") return "<span style='padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;background:rgba(34,197,94,.2);color:#22c55e'>Yüksek</span>";
  if(v==="orta") return "<span style='padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;background:rgba(251,191,36,.2);color:#fbbf24'>Orta</span>";
  if(v==="dusuk") return "<span style='padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;background:rgba(239,68,68,.15);color:#ef4444'>Düşük</span>";
  return "<span style='color:#5a566a'>—</span>";
}

function DB(v){
  var m={yeni:"rgba(96,165,250,.2)|#60a5fa|Yeni",araniyor:"rgba(251,191,36,.2)|#fbbf24|Aranıyor",ilgileniyor:"rgba(34,197,94,.2)|#22c55e|İlgileniyor",teklif:"rgba(201,169,110,.2)|#C9A96E|Teklif",satildi:"rgba(168,85,247,.2)|#a855f7|SATILDI",vazgecti:"rgba(239,68,68,.15)|#ef4444|Vazgeçti"};
  var e=(m[v]||m.yeni).split("|");
  return "<span style='padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;background:"+e[0]+";color:"+e[1]+"'>"+e[2]+"</span>";
}
...
function LD(){
  ...
  var cq=q.replace(/&select=[^&]*/,"&select=id").replace(/&limit=[^&]*/,"").replace(/&offset=[^&]*/,"");
  Promise.all([
    ...
    _d=rs[0]||[];
    var tot=rs[1]?parseInt(rs[1].split("/")[1]||0):0;
    RD(tot);
  });
}
...
function RD(tot){
  ...
  for(var i=0;i<_d.length;i++){
    ...
    html+="<tr data-id='"+c.id+"' style='border-bottom:1px solid rgba(255,255,255,.04)' onmouseover="this.style.background='rgba(255,255,255,.025)'" onmouseout="this.style.background=''">";
    html+="<td style='padding:6px 10px;text-align:center'><input type='checkbox' class='am-ck' data-id='"+c.id+"' "+chk+" onchange="_amTog('"+c.id+"')" style='accent-color:#C9A96E;width:14px;height:14px'></td>";
    html+="<td style='padding:6px 10px'><span onclick="_amDet('"+c.id+"','"+nm+"')" style='color:#f0ede8;font-weight:600;cursor:pointer;font-size:13px'>"+nm+"</span>";
    if(c.etiket) html+="<span style='background:rgba(201,169,110,.15);color:#C9A96E;border-radius:3px;padding:1px 5px;font-size:10px;margin-left:4px'>"+c.etiket+"</span>";
    if(c.personel_notu) html+="<span style='color:#60a5fa;font-size:10px;margin-left:3px' title='Not var'>📝</span>";
    html+="</td>";
    html+="<td style='padding:6px 10px;font-size:11px;color:#5a566a'>"+(ph||"—")+"</td>";
    html+="<td style='padding:6px 10px;text-align:center'><span style='font-size:15px;font-weight:700;color:"+bc+"'>"+(sk>0?bp:"—")+"</span></td>";
...
function PG(tot){
  ...
  var s=Math.max(1,_p-2),e=Math.min(pages,s+4);
  if(e-s<4) s=Math.max(1,e-4);
  var b="<span style='color:#5a566a;font-size:12px;margin-right:8px'>"+tot+" kayıt</span>";
  if(s>1) b+="<button onclick='_amPg(1)' style='background:#21212e;color:#a8a4b0;border:1px solid rgba(255,255,255,.08);border-radius:5px;padding:4px 8px;cursor:pointer;font-size:12px;margin:2px'>1</button><span style='color:#5a566a'>…</span>";
  for(var i=s;i<=e;i++) b+="<button onclick='_amPg("+i+")' style='"+(i===_p?"background:#C9A96E;color:#000;font-weight:700":"background:#21212e;color:#a8a4b0")+";border:1px solid rgba(255,255,255,.08);border-radius:5px;padding:4px 8px;cursor:pointer;font-size:12px;margin:2px'>"+i+"</button>";
  if(e<pages) b+="<span style='color:#5a566a'>…</span><button onclick='_amPg("+pages+")' style='background:#21212e;color:#a8a4b0;border:1px solid rgba(255,255,255,.08);border-radius:5px;padding:4px 8px;cursor:pointer;font-size:12px;margin:2px'>"+pages+"</button>";
  el.innerHTML=b+"<span style='color:#5a566a;font-size:12px;margin-left:8px'>Sayfa "+_p+"/"+pages+"</span>";
}
...
function SLB(){
  ...
  if(cnt) cnt.textContent=_sel.size+" seçildi";
}
...
window._amFil=function(){
  _f={
    ilgi:document.getElementById("am-filgi")&&document.getElementById("am-filgi").value,
    skor:document.getElementById("am-fskor")&&document.getElementById("am-fskor").value,
    sd:document.getElementById("am-fsd")&&document.getElementById("am-fsd").value,
    asgn:document.getElementById("am-fasgn")&&document.getElementById("am-fasgn").value
  };
  _p=1;LD();
};
...
window._amExp=function(){
  var ids=Array.from(_sel);
  var rows=ids.length?_d.filter(function(c){return ids.includes(c.id);}):_d;
  var csv="\uFEFFAd Soyad,Telefon,Skor,İlgi,Durum,Özet,Personel,Tarih\n";
  csv+=rows.map(function(c){
    return [(c.customers&&c.customers.full_name||""),(c.customers&&c.customers.phone||""),c.skor||0,c.ilgi_seviyesi||"",c.satis_durumu||"","\""+(c.summary||"").replace(/"/g,"''")+"\"",c.assigned_name||"",(c.created_at||"").substring(0,10)].join(",");
  }).join("\n");
  var a=document.createElement("a");
  a.href=URL.createObjectURL(new Blob([csv],{type:"text/csv;charset=utf-8"}));
  a.download="aramalar_"+(new Date().toISOString().substring(0,10))+".csv";
  a.click();
  T((ids.length||rows.length)+" satır export edildi");
};
...
window._amAsgn=function(){
  ...
  fetch(SB+"/rest/v1/users?select=id,full_name&order=full_name",{headers:H}).then(function(r){return r.json();}).then(function(ud){
    var m=document.createElement("div");m.id="am-am";
    ...
    document.getElementById("am-aok").onclick=function(){
      var sel=document.getElementById("am-aus");
      var uid=sel.value,un=sel.selectedOptions[0]&&sel.selectedOptions[0].text;
      if(!uid){T("Personel seçin!","w");return;}
      var ids=Array.from(_sel);var done=0;var total=ids.length;
      var batches=[];
      for(var i=0;i<ids.length;i+=10) batches.push(ids.slice(i,i+10));
      var proc=function(idx){
        if(idx>=batches.length){m.remove();_sel.clear();T(done+" arama "+un+"a atandı");LD();return;}
        Promise.all(batches[idx].map(function(id){
          return fetch(SB+"/rest/v1/calls?id=eq."+id,{method:"PATCH",headers:HC,body:JSON.stringify({assigned_to:uid,assigned_name:un,satis_durumu:"yeni"})}).then(function(r){if(r.status===204)done++;});
        })).then(function(){proc(idx+1);});
      };
      proc(0);
    };
  });
...
window._amUpdSD=function(cid,val){
  fetch(SB+"/rest/v1/calls?id=eq."+cid,{method:"PATCH",headers:HC,body:JSON.stringify({satis_durumu:val})}).then(function(){
    T(val==="satildi"?"🎉 SATILDI!":"Durum güncellendi");LD();
  });
};

window._amTag=function(cid,tag){
  var c=null;for(var i=0;i<_d.length;i++){if(_d[i].id===cid){c=_d[i];break;}}
  var nt=(c&&c.etiket===tag)?null:tag;
  fetch(SB+"/rest/v1/calls?id=eq."+cid,{method:"PATCH",headers:HC,body:JSON.stringify({etiket:nt})}).then(function(){
    T(nt?"Etiket: "+nt:"Etiket kaldırıldı");
    var dp=document.getElementById("am-dp");if(dp)dp.remove();
    LD();
  });
};

window._amNote=function(cid){
  var n=document.getElementById("am-dpnote")&&document.getElementById("am-dpnote").value||"";
  fetch(SB+"/rest/v1/calls?id=eq."+cid,{method:"PATCH",headers:HC,body:JSON.stringify({personel_notu:n})}).then(function(){
    T("Not kaydedildi ✓");
  });
};
...
window._amDet=function(cid,cn){
  ...
  fetch(SB+"/rest/v1/calls?id=eq."+cid+"&select=*,customers(full_name,phone)",{headers:H}).then(function(r){return r.json();}).then(function(data){
    var c=data&&data[0];
    if(!c){
      return fetch(SB+"/rest/v1/calls?agent_name=eq.Ayse&select=*,customers(full_name,phone)&order=created_at.desc&limit=200",{headers:H}).then(function(r){return r.json();}).then(function(all){
        var found=null;
        for(var i=0;i<all.length;i++){if(all[i].customers&&all[i].customers.full_name&&all[i].customers.full_name.substring(0,5).toLowerCase()===cn.substring(0,5).toLowerCase()){found=all[i];break;}}
        renderDet(found,cn,cid);
      });
    }
    renderDet(c,cn,cid);
  });
...
function renderDet(c,cn,cid){
  ...
  for(var i=0;i<trLines.length;i++){
    var l=trLines[i];
    if(!l.trim()) continue;
    if(l.indexOf("AI:")===0) tr+="<div style='margin:3px 0;padding:5px 9px;background:rgba(201,169,110,.1);border-radius:5px;color:#C9A96E;font-size:12px;line-height:1.4'><b>Ayşe:</b> "+l.slice(3).trim()+"</div>";
    else if(l.indexOf("User:")===0) tr+="<div style='margin:3px 0;padding:5px 9px;background:rgba(255,255,255,.05);border-radius:5px;color:#f0ede8;font-size:12px;line-height:1.4'><b>Müşteri:</b> "+l.slice(5).trim()+"</div>";
  }
  var sdOpts=["yeni|Yeni","araniyor|Aranıyor","ilgileniyor|İlgileniyor","teklif|Teklif Verildi","satildi|✅ SATILDI","vazgecti|Vazgeçti"];
  var selOpts="";for(var i=0;i<sdOpts.length;i++){var parts=sdOpts[i].split("|");selOpts+="<option value='"+parts[0]+"'"+(sd===parts[0]?" selected":"")+">"+parts[1]+"</option>";}
  var tags=["🔥 Sıcak","⭐ VIP","🔄 Geri Ara","💰 Yüksek Bütçe","⚡ Acil"];
  var tagBtns="";
  for(var i=0;i<tags.length;i++){
    var act=(c.etiket||"")===tags[i];
    tagBtns+="<button onclick="_amTag('"+cid+"','"+tags[i]+"')" style='background:"+(act?"rgba(201,169,110,.3)":"rgba(255,255,255,.05)")+";color:"+(act?"#C9A96E":"#a8a4b0")+";border:1px solid rgba(255,255,255,.08);border-radius:5px;padding:4px 8px;font-size:11px;cursor:pointer'>"+tags[i]+"</button>";
  }
  b.innerHTML=
    ...
    +"<div style='margin-bottom:10px'><div style='font-size:10px;color:#a8a4b0;margin-bottom:5px'>SATIŞ DURUMU</div><select id='am-dpsd' onchange="_amUpdSD('"+cid+"',this.value)" style='width:100%;background:#21212e;color:#f0ede8;border:1px solid rgba(255,255,255,.1);border-radius:7px;padding:8px;font-size:13px'>"+selOpts+"</select></div>"
    ...
    +(c.itiraz&&c.itiraz!=="-"?"<div style='background:rgba(251,191,36,.08);border-radius:7px;padding:8px;margin-bottom:8px;font-size:12px;color:#fbbf24'>⚠️ "+c.itiraz+"</div>":"")
    ...
    +"<button onclick="_amNote('"+cid+"')" style='width:100%;margin-top:5px;background:#C9A96E;color:#000;border:none;border-radius:7px;padding:8px;font-weight:700;cursor:pointer;font-size:12px'>Notu Kaydet</button></div>"
    +"<div style='font-size:10px;color:#a8a4b0;margin-bottom:7px;border-top:1px solid rgba(255,255,255,.06);padding-top:10px'>KONUŞMA</div>"
    +(tr||"<p style='color:#5a566a;font-size:12px;text-align:center;padding:16px'>Transkript yok</p>");
}

window.updateSD=function(cid,val){
  fetch(SB+"/rest/v1/calls?id=eq."+cid,{method:"PATCH",headers:HC,body:JSON.stringify({satis_durumu:val})}).then(function(){T(val==="satildi"?"🎉 SATILDI!":"Güncellendi");});
};
...
window.loadSalesPanel=function(){
  ...
  fetch(q,{headers:H}).then(function(r){return r.json();}).then(function(rows){
    ...
    if(!rows.length){body.innerHTML="<p style='color:#5a566a;text-align:center;padding:40px'>Bu personele atanmış lead yok</p>";return;}
    var html="";
    for(var i=0;i<rows.length;i++){
      ...
      var ph=(c.customers&&c.customers.phone||"").replace(/[^0-9+]/g,"");
      var sd=c.satis_durumu||"yeni";
      var sdMap={satildi:"rgba(168,85,247,.2)",ilgileniyor:"rgba(34,197,94,.2)",teklif:"rgba(251,191,36,.2)",vazgecti:"rgba(239,68,68,.15)",araniyor:"rgba(251,191,36,.15)"};
      var bg=sdMap[sd]||"rgba(96,165,250,.2)";
      var lbMap={satildi:"SATILDI",ilgileniyor:"İlgileniyor",teklif:"Teklif",vazgecti:"Vazgeçti",araniyor:"Aranıyor",yeni:"Yeni"};
      var lb=lbMap[sd]||"Yeni";
      var nm=c.customers&&c.customers.full_name||"?";
      var sm=c.summary?c.summary.substring(0,180)+(c.summary.length>180?"...":""):"";
      var sdOpts="";var sdVals=["yeni|Yeni","araniyor|Aranıyor","ilgileniyor|İlgileniyor","teklif|Teklif","satildi|SATILDI","vazgecti|Vazgeçti"];
      for(var j=0;j<sdVals.length;j++){var p=sdVals[j].split("|");sdOpts+="<option value='"+p[0]+"'"+(sd===p[0]?" selected":"")+">"+p[1]+"</option>";}
      html+="<div class='stcard' data-id='"+c.id+"' style='background:#1a1a24;border:1px solid rgba(255,255,255,.08);border-radius:10px;padding:14px;margin-bottom:10px'>";
      html+="<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:8px'><span style='font-size:14px;font-weight:700;color:#f0ede8'>"+nm+"</span><span style='padding:3px 8px;border-radius:4px;font-size:11px;font-weight:600;background:"+bg+"'>"+lb+"</span></div>";
      ...
      if(sm) html+="<div style='font-size:12px;color:#a8a4b0;line-height:1.5;background:rgba(255,255,255,.03);border-radius:6px;padding:8px;margin-bottom:8px'>"+sm+"</div>";
      if(c.itiraz&&c.itiraz!=="-") html+="<div style='font-size:11px;color:#fbbf24;margin-bottom:4px'>⚠️ "+c.itiraz+"</div>";
      if(c.takip_notu) html+="<div style='font-size:11px;color:#60a5fa;margin-bottom:6px'>📌 "+c.takip_notu+"</div>";
      html+="<select onchange="updateSD(this.dataset.c,this.value)" data-c='"+c.id+"' style='background:#21212e;color:#f0ede8;border:1px solid rgba(255,255,255,.1);border-radius:6px;padding:4px 8px;font-size:11px'>"+sdOpts+"</select></div>";
    }
    body.innerHTML=html;
  });
};
...
window.showBulkAssign=function(){
  fetch(SB+"/rest/v1/users?select=id,full_name&order=full_name",{headers:H}).then(function(r){return r.json();}).then(function(ud){
    ...
    m.innerHTML="<div style='background:#1a1a24;border:1px solid rgba(201,169,110,.3);border-radius:14px;padding:28px;width:500px;max-height:85vh;overflow-y:auto'>"
    +"<div style='font-size:16px;font-weight:700;color:#f0ede8;margin-bottom:18px'>⚡ Toplu Atama</div>"
    +"<div style='display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px'>"
    +"<div><label style='font-size:11px;color:#a8a4b0;display:block;margin-bottom:5px'>MİNİMUM SKOR</label><select id='ba-minskor' style='width:100%;background:#21212e;color:#f0ede8;border:1px solid rgba(255,255,255,.12);border-radius:8px;padding:9px;font-size:13px'><option value='1'>1+</option><option value='3'>3+</option><option value='5' selected>5+</option><option value='7'>7+</option></select></div>"
    +"<div><label style='font-size:11px;color:#a8a4b0;display:block;margin-bottom:5px'>MEVCUT ATAMA</label><select id='ba-filter' style='width:100%;background:#21212e;color:#f0ede8;border:1px solid rgba(255,255,255,.12);border-radius:8px;padding:9px;font-size:13px'><option value='unassigned' selected>Sadece atanmamış</option><option value='all'>Tümünü yeniden dağıt</option></select></div>"
    +"</div><div style='margin-bottom:16px'><div style='display:flex;justify-content:space-between;margin-bottom:8px'><label style='font-size:11px;color:#a8a4b0'>PERSONELLER</label><button onclick="document.querySelectorAll('.ba-user').forEach(function(c){c.checked=true;})" style='font-size:11px;color:#C9A96E;background:none;border:none;cursor:pointer'>Tümünü Seç</button></div>"
    +"<div style='display:grid;grid-template-columns:1fr 1fr;gap:8px'>"+ud.map(function(u){return"<label style='display:flex;align-items:center;gap:8px;background:#21212e;border:1px solid rgba(255,255,255,.08);border-radius:8px;padding:8px 10px;cursor:pointer'><input type='checkbox' class='ba-user' value='"+u.id+"' data-name='"+u.full_name+"' style='accent-color:#C9A96E;width:14px;height:14px'><span style='font-size:13px;color:#f0ede8'>"+u.full_name+"</span></label>";}).join("")+"</div></div>"
    +"<div id='ba-preview' style='background:#21212e;border-radius:8px;padding:10px;margin-bottom:16px;font-size:12px;color:#5a566a;text-align:center'>Önizleme için Hesapla butonuna bas</div>"
...
window.calcBulkAssign=function(){
  var ms=parseInt(document.getElementById("ba-minskor").value);
  var fm=document.getElementById("ba-filter").value;
  var sel=[];var chks=document.querySelectorAll(".ba-user:checked");
  for(var i=0;i<chks.length;i++) sel.push({id:chks[i].value,name:chks[i].dataset.name});
  if(!sel.length){T("En az 1 personel seçin!","w");return;}
  var pv=document.getElementById("ba-preview");
  pv.innerHTML="<span style='color:#C9A96E'>Hesaplanıyor...</span>";
...
window.doBulkAssign=function(){
  var calls=window._bulkCalls,users=window._bulkUsers;
  if(!calls||!users) return;
  var btn=document.getElementById("ba-confirm");if(btn){btn.textContent="Dağıtılıyor...";btn.disabled=true;}
  var done=0,stats={};
  for(var i=0;i<users.length;i++) stats[users[i].name]=0;
  var batches=[];
  for(var i=0;i<calls.length;i+=5) batches.push(calls.slice(i,i+5));
  var proc=function(bi){
    if(bi>=batches.length){
      var modal=document.getElementById("ba-modal");if(modal)modal.remove();
      var t=document.createElement("div");
      t.style.cssText="position:fixed;top:20px;right:20px;background:#1a1a24;border:2px solid #22c55e;border-radius:12px;padding:16px 20px;z-index:99999;min-width:260px;box-shadow:0 8px 32px rgba(0,0,0,.5)";
      var sh="";var ks=Object.keys(stats);
      for(var i=0;i<ks.length;i++) sh+="<div style='display:flex;justify-content:space-between;margin-top:4px'><span style='color:#a8a4b0;font-size:12px'>"+ks[i]+"</span><span style='color:#22c55e;font-size:12px;font-weight:700'>"+stats[ks[i]]+" arama</span></div>";
    ...
    Promise.all(batches[bi].map(function(c,j){
      var user=users[(bi*5+j)%users.length];
      return fetch(SB+"/rest/v1/calls?id=eq."+c.id,{method:"PATCH",headers:HC,body:JSON.stringify({assigned_to:user.id,assigned_name:user.name,satis_durumu:"yeni"})}).then(function(r){if(r.status===204){done++;stats[user.name]=(stats[user.name]||0)+1;}});
    })).then(function(){proc(bi+1);});
  };
  proc(0);
};
...
function addNav(){
  ...
  if(!document.getElementById("am-btn")){
  ...
  if(!document.getElementById("st-nav")){
    ...
    b2.onclick=function(){
      var pg=document.getElementById("st-pg");
      if(pg){pg.style.display=pg.style.display==="none"?"block":"none";return;}
      fetch(SB+"/rest/v1/users?select=id,full_name&order=full_name",{headers:H}).then(function(r){return r.json();}).then(function(users){
        ...
        pg=document.createElement("div");pg.id="st-pg";
        pg.style.cssText="position:fixed;top:0;left:0;right:0;bottom:0;background:#0d0d12;z-index:8000;overflow-y:auto;padding:20px";
  ...
  if(!document.getElementById("ba-btn")){
    var analiz=document.querySelector("[onclick*='syncVapi'],[onclick*='analiz']");
    if(analiz){
      var b3=document.createElement("button");b3.id="ba-btn";b3.textContent="⚡ Toplu Ata";
      b3.style.cssText="background:linear-gradient(135deg,#C9A96E,#a07840);color:#000;border:none;border-radius:8px;padding:7px 14px;font-size:12px;font-weight:700;cursor:pointer;margin-left:8px";
      b3.onclick=function(){showBulkAssign();};
      analiz.parentElement.insertBefore(b3,analiz.nextSibling);
    }
  }
}
new MutationObserver(function(){addNav();}).observe(document.body,{childList:true,subtree:true});
...
window.showAramaMerkezi=window.showAM=function(){
  ...
  +"<select id='am-fasgn' onchange='_amFil()' style='background:#21212e;color:#f0ede8;border:1px solid rgba(255,255,255,.1);border-radius:6px;padding:5px 8px;font-size:12px'><option value='all'>Tüm Atama</option><option value='yes'>Atanmış</option><option value='no'>Atanmamış</option></select>"
  ...
  document.body.appendChild(pg);
  _p=1;_sel.clear();_f={};_s="skor.desc.nullslast";
  LD();
};
})();